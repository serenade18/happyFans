...
class AddFundsController extends Controller
{
  ...
  public function sendCoinpayments()
  {
    try {
      ...
      $invoice = $coinPayments->createInvoice([
        'currency' => $this->settings->currency_code,
        'amount' => [
          'total' => $amountFixed,
        ],
        ...
        'successUrl' => route('paymentProcess'),
        'cancelUrl' => url('my/wallet'),

        'webhook' => [
          'notifications' => $urlIPN
        ],
      ]);

      return response()->json([
        'success' => true,
        'url' => $invoice['invoices'][0]['checkoutLink'],
      ]);
    } catch (\Exception $e) {
      return response()->json([
        'success' => false,
  ...
  public function coinPaymentsIPN(Request $request)
  {
    ...
    if (!$coinPayments->validateWebhook($request)) {
      logger()->warning('Invalid CoinPayments webhook received');
    ...
    try {
      ...
      // Use case-insensitive comparison as recommended in the documentation
      $eventTypeLower = strtolower($eventType);
      $txnId = $data['id'];

      switch ($eventTypeLower) {
        case 'invoice.completed':
        case strtolower('invoiceCompleted'):

          if (Deposits::query()->whereTxnId($request->txnId)->doesntExist()) {
            // Insert Deposit
            $this->deposit($user->id, $txnId, $request->amountOriginal, 'Coinpayments', $request->taxes ?? null);
      ...
      logger()->error('Error processing CoinPayments webhook: ' . $e->getMessage());
   ...
   * @return Response
   */
  protected function sendCoinpaymentsLegacy()
  {
    // Get Payment Gateway
    $payment = PaymentGateways::whereName('Coinpayments')->firstOrFail();
  ...
  // CoinPaments IPN
  public function coinPaymentsIPNLegacy(Request $request)
  {
    // Get Payment Gateway
    $payment = PaymentGateways::whereName('Coinpayments')->firstOrFail();
